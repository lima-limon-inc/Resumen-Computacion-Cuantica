...
Fue por primera vez realizado con atomos de plata calentados en un horno. Estos atomos eran enviados a traves de un campo magnetico, y luego chocaban contra una placa.

La prediccion original decia que los atomos tenian que formar una linea recta en la plca de metal, teniendo una distibucion normal.
Sin embargo, vieron que los atomos quedaban en dos de las puntas de la placa.

Esto mostro la propiedad de *spin* propia de los electrones: Esta le da al electron un momento angular intrinseco, similar al efecto de girar un obejto.
...
R_z(\delta) \ket 1 = e^{i \cdot \delta} \ket 1 \\
$$
\end{center}
#+end_export
...
*NOTA*: Es re confusa esa expresion, es mas facil pensarlo como:
- Si esta a en 1, hay swap. Sino no.

El + es "or" y el ${ \cdot  }$ es "and".
...
La idea de este algoritmo es de enviar dos bits de informacion usando un solo qubit.
[[file:images/codificacion-densa.png]]

*** Pasos:
1. Se separan dos qubits entrelazados
...
* Estados entrelazados
SOn los estados que no se pueden escribir como el producto tensorial de 2 estados de 1 qubit cada uno

** Entropia de Von Neuman
Si ${ p_i }$ es el autovalor asociado a ${ \hat \rho }$  asociado a ${ \ket i }$  entonces:
${ S = - \displaystyle{\sum^{N}_{i=i}} p_i \log_2 (p_i)  }$

Si S = 0, es un estado puro. Si S = 1, es entrelazamiento maximo.
* Estados mezcla
Es cuando el estado cuantico de un sistema no se puede describir con *un unico vector de estado*. Esto es como una mezcladora de proba. Vos juntas los qubits que estan en distintas estados y "las pones juntas".
Son varios estados, cada uno con una probabilidad distinta.

O esta en un estado, o en otro. No puede estar en ambos.
** Operador de densidad
Es la forma de describir estados mezcla.
${ \hat \rho (t) = \displaystyle{\sum^{N}_{n,p}} c_n(t) c_n^*(t) \ket{\psi_n} \bra{\psi_n}  }$

*** Propiedades
- La traza de la matriz asociada mide 1
*** Representacion matricial
El elemento en las coordenadas ${ n, p }$ es:

${ \rho_{np} = \braket{\psi_n|\Psi(t)} \braket{\Psi(t)|\psi_p} }$

** Matriz densidad reducida de un sistema bipartito que tiene componentes A y B
La matriz de densidad reducida para componentes de un sistema bipartito que tiene componentes A y B se determina usando la traza parcial

${ \hat \rho = Tr_B(\hat \rho_{AB}) }$
${ \hat \rho = Tr_A(\hat \rho_{AB}) }$

EL operador estadistico del sistema es:
${ \hat \rho_{AB} = \ket{\psi_{AB}} \bra{\psi_{AB}} = \ket{A_1 B_1} \bra{A_2B_2} }$

Los parciales
${ \hat \rho_A = Tr_A(\hat \rho_{AB}) = \ket{A_1}\bra{A_2} \braket{B_1 | B_2} }$
${ \hat \rho_B = Tr_B(\hat \rho_{AB}) = \ket{B_1}\bra{B_2} \braket{A_1 | A_2} }$
